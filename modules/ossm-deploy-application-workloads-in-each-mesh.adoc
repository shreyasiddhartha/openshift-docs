...
. Deploy the `sleep` and `httpbin` applications by running the following commands:
+
[source,terminal]
----
$ oc apply -n app-ns-1 \
...
. Create a second applicatiom namespaces called `app-ns-2` by running the following command:
+
[source,terminal]
----
...
. Deploy the `sleep` and `httbin` applications to both namespaces by running the following commands:
+
[source,terminal]
----
$ oc apply -n app-ns-2 \
...
. Optional: You can use the following command to wait for deployment to be ready:
+
[source,terminal]
----
...
.. Verify the workloads assigned to the control plane in `istio-system-1` by running the following command:
+
[source,terminal]
----
...
.. Verify the workloads assigned to the control plane in `istio-system-2` by running the following command:
+
[source,terminal]
----
...
. Verify that application connectivity is restricted to workloads within their respective mesh.
