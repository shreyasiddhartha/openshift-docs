...
spec:
  ...
  - path: /stats/prometheus
    ...
    - sourceLabels: ["__meta_kubernetes_pod_label_app_kubernetes_io_name","__meta_kubernetes_pod_label_app"] # <2>
      ...
      regex: "(.+);.*|.*;(.+)"
      replacement: "${1}${2}"
    - sourceLabels: ["__meta_kubernetes_pod_label_app_kubernetes_io_version","__meta_kubernetes_pod_label_version"] # <3>
      separator: ";"
      targetLabel: "version"
      action: replace
      regex: "(.+);.*|.*;(.+)"
    ...
    - action: replace
      replacement: "the-mesh-identification-string" # <4>
      targetLabel: mesh_id 
----
...
<3> Specify the `version` label from `app.kubernetes.io/version` or fallback to `version`.
<4> Specify the actual mesh ID.
